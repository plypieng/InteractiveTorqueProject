...
from app.utils.file_security import is_safe_path
...
def generate_pdf(csv_file_path, cutoff=0.1):
    try:
        if not is_safe_path(Config.ALLOWED_DIRECTORY, csv_file_path):
            logging.warning(f"Attempt to access invalid file path: {csv_file_path}")
            return
